...
.. note::
   ...
   The Python and NumPy indexing ``[]`` and attribute ``.`` operators provide 
   a quick and easy access to pandas data structures across a wide range of use
   cases making interactive work intuitive. However, since the type of data to
   be accessed isn't known in advance, using standard operators directly has 
   optimization limits. For production code, we recommend that you take advantage
   of the optimized pandas data access methods elucidated in this chapter.
